...
import presentation.formbeans.oEducationSys.Action_view_education_Form;
 ...
 *
 */
public class Action_view_education extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Action_view_education_Form action_view_education_Form = (Action_view_education_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request, action_view_education_Form);

		/** 4.  We obtain the initial POJO. */

		Education education = UseCaseFacade.ObtainEducation(usercode, action_view_education_Form.getEducationcode());
		...
		fbManager = new FormBeanManager(request, Action_view_education_Form.class);

		/** 6.2.  We add the current POJO merged with the nested-pojo-form-bean to the manager. */

		fbManager.setAttribute(education, "education_Form", Education_Form.class, action_view_education_Form.getEducation_Form());

		/** 6.3.  We add the container of the manager to the request. */

		request.setAttribute("oEducationSys__action_view_education_Form", fbManager.getContainer());
