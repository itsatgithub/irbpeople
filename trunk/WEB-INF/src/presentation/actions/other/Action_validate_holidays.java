 ...
 *
 */
public class Action_validate_holidays extends Action {
...
FormBeanManager fbManager=new FormBeanManager(request, action_validate_holidays_Form);
	...
	try {
		irbholiday = UseCaseFacade.ObtainIrbholiday(usercode, action_validate_holidays_Form.getIrbholidaycode());
	} catch (Exception e) {
	...
	if(irbholiday == null) {
		UserUtils.addSessionSavedAuditMessage(request, "error.no-existe");
		return NavigationFunctions.findForward(request, mapping, "error");
	} else {
		if(irbholiday.getValidationcode() == null || irbholiday.getValidationcode().equals("")) {
			UserUtils.addSessionSavedAuditMessage(request, "errors.no-validation");
			return NavigationFunctions.findForward(request, mapping, "error");
		}
		if(irbholiday.getValidationcode().compareTo(action_validate_holidays_Form.getValidationcode()) != 0) {
			UserUtils.addSessionSavedAuditMessage(request, "errors.validation-code-incorrecto");
			return NavigationFunctions.findForward(request, mapping, "error");
		}
	}
	
	Irbholiday_Form irbholiday_Form = action_validate_holidays_Form.getIrbholiday_Form();
...
fbManager=new FormBeanManager(request, Action_validate_holidays_Form.class);
