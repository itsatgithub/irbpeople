 ...
 *
 */
public class Action_view_payment extends Action {
...
Action_view_payment_Form action_view_payment_Form=(Action_view_payment_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_view_payment_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Payment payment = UseCaseFacade.ObtainPayment(usercode, action_view_payment_Form.getPaymentcode());
...
fbManager=new FormBeanManager(request, Action_view_payment_Form.class);
	...
	fbManager.setAttribute(payment, "payment_Form", Payment_Form.class, action_view_payment_Form.getPayment_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oPaymentSys__action_view_payment_Form", fbManager.getContainer());
