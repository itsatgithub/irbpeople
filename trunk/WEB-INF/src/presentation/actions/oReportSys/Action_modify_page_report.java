 ...
 *
 */
public class Action_modify_page_report extends Action {
...
Action_modify_page_report_Form action_modify_page_report_Form=(Action_modify_page_report_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_modify_page_report_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Report report = UseCaseFacade.ObtainReport(usercode, action_modify_page_report_Form.getReportcode());
...
fbManager=new FormBeanManager(request, Action_modify_page_report_Form.class);
	...
	fbManager.setAttribute(report, "report_Form", Report_Form.class, action_modify_page_report_Form.getReport_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oReportSys__action_modify_page_report_Form", fbManager.getContainer());

	/** 6.4.  We add the values of all (if any) of the support tables which will be used in the jsp (for selectables) to the request. */	
	
	

action_modify_page_report_Form.getReport_Form().getAuthor().setUsuariocode(usercode);
