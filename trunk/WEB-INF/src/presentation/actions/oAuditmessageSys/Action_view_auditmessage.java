 ...
 *
 */
public class Action_view_auditmessage extends Action {
...
Action_view_auditmessage_Form action_view_auditmessage_Form=(Action_view_auditmessage_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_view_auditmessage_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Auditmessage auditmessage = UseCaseFacade.ObtainAuditmessage(usercode, action_view_auditmessage_Form.getAuditmessagecode());
...
fbManager=new FormBeanManager(request, Action_view_auditmessage_Form.class);
	...
	fbManager.setAttribute(auditmessage, "auditmessage_Form", Auditmessage_Form.class, action_view_auditmessage_Form.getAuditmessage_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oAuditmessageSys__action_view_auditmessage_Form", fbManager.getContainer());
...
Pair<Integer, List<Usuario>> _selec_usuario= UseCaseFacade.ObtenerUsuariosActivos(usercode,  new ListConfigurator());
/** 6.4.2.  We copy the values to a list of formbeans. */

List<UsuarioForm> _selec_usuarioForm = ExtendedBeanUtils.copyPropertiesToFormBean(_selec_usuario.getSecond(), locale, UsuarioForm.class);
/** 6.4.3.  We sort the list. */
Collections.sort(_selec_usuarioForm, new ToStringComparator());
/** 6.4.4.  We put the values to the request, so the jsp can get them. */

request.setAttribute("_selec_usuario", _selec_usuarioForm);
/** 6.4.5.  We obtain the values form the business logic. */

Pair<Integer, List<Auditmessagetype>> _selec_auditmessagetype= UseCaseFacade.ObtainAllAuditmessagetype(usercode,  new ListConfigurator());
/** 6.4.6.  We copy the values to a list of formbeans. */

List<Auditmessagetype_Form> _selec_auditmessagetype_Form = ExtendedBeanUtils.copyPropertiesToFormBean(_selec_auditmessagetype.getSecond(), locale, Auditmessagetype_Form.class);
/** 6.4.7.  We sort the list. */
Collections.sort(_selec_auditmessagetype_Form, new ToStringComparator());
/** 6.4.8.  We put the values to the request, so the jsp can get them. */

request.setAttribute("_selec_auditmessagetype", _selec_auditmessagetype_Form);
