...
import presentation.formbeans.oAcademic_infoSys.Action_view_academic_info_Form;
 ...
 *
 */
public class Action_view_academic_info extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Action_view_academic_info_Form action_view_academic_info_Form = (Action_view_academic_info_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request, action_view_academic_info_Form);

		/** 4.  We obtain the initial POJO. */

		Academic_info academic_info = UseCaseFacade.ObtainAcademic_info(usercode, action_view_academic_info_Form.getAcademic_infocode());
		...
		fbManager = new FormBeanManager(request, Action_view_academic_info_Form.class);
		...
		fbManager.setAttribute(
				...
				action_view_academic_info_Form.getAcademic_info_Form());

		/** 6.3.  We add the container of the manager to the request. */

		request.setAttribute("oAcademic_infoSys__action_view_academic_info_Form", fbManager.getContainer());
		...
		Pair<Integer, List<Type_of_study>> _type_of_study = UseCaseFacade.ObtainAllType_of_study(usercode, new ListConfigurator());
		/** 6.4.6.  We copy the values to a list of formbeans. */

		List<Type_of_study_Form> _select_type_of_studyForm = ExtendedBeanUtils.copyPropertiesToFormBean(_type_of_study.getSecond(), locale, Type_of_study_Form.class);
		/** 6.4.7.  We sort the list. */
		Collections.sort(_select_type_of_studyForm, new ToStringComparator());
		/** 6.4.8.  We put the values to the request, so the jsp can get them. */

		request.setAttribute("_selec_type_of_study", _select_type_of_studyForm);
