 ...
 *
 */
public class Action_manage_my_own_personal_grant_concessions extends Action {
...
Action_manage_my_own_personal_grant_concessions_Form action_manage_my_own_personal_grant_concessions_Form=(Action_manage_my_own_personal_grant_concessions_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_manage_my_own_personal_grant_concessions_Form);
...
fbManager=new FormBeanManager(request, Action_manage_my_own_personal_grant_concessions_Form.class);
	...
	fbManager.setAttribute(personal, "personal_Form", Personal_Form.class, action_manage_my_own_personal_grant_concessions_Form.getPersonal_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oPersonalSys__action_manage_my_own_personal_grant_concessions_Form", fbManager.getContainer());
	...
	/** 6.5.2.  We obtain the list of objects form the Business logic */
			...
			Pair<Integer,List<Grant_concession>> _list_grant_concession=UseCaseFacade.ObtainAllIgrant_concession_personalFromPersonal(usercode,action_manage_my_own_personal_grant_concessions_Form.getPersonal_Form().getPersonalcode(),filter.obtainListConfigurator(locale, false));
	
	/** 6.5.3.  We obtain copy the list of pojos to a list of FormBeans */
	
	List<Grant_concession_Form> viewlistElements =ExtendedBeanUtils.copyPropertiesToFormBean(_list_grant_concession.getSecond(), locale, Grant_concession_Form.class);
