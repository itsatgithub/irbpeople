 ...
 *
 */
public class Action_manage_personal_holidays extends Action {
...
Action_manage_personal_holidays_Form action_manage_personal_holidays_Form=(Action_manage_personal_holidays_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_manage_personal_holidays_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Personal personal = UseCaseFacade.ObtainPersonal(usercode, action_manage_personal_holidays_Form.getPersonalcode());
...
fbManager=new FormBeanManager(request, Action_manage_personal_holidays_Form.class);
	...
	fbManager.setAttribute(personal, "personal_Form", Personal_Form.class, action_manage_personal_holidays_Form.getPersonal_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oPersonalSys__action_manage_personal_holidays_Form", fbManager.getContainer());
	...
	/** 6.5.2.  We obtain the list of objects form the Business logic */
			...
			Pair<Integer,List<Holiday>> _list_holiday=UseCaseFacade.ObtainAllIholiday_personalFromPersonal(usercode,action_manage_personal_holidays_Form.getPersonal_Form().getPersonalcode(),filter.obtainListConfigurator(locale, true));
	
	/** 6.5.3.  We obtain copy the list of pojos to a list of FormBeans */
	
	List<Holiday_Form> viewlistElements =ExtendedBeanUtils.copyPropertiesToFormBean(_list_holiday.getSecond(), locale, Holiday_Form.class);
	
	request.setAttribute("viewlistElements", viewlistElements);
	/** 6.5.4.  We put the 'real' number of elements (without pagination) */
		
		request.setAttribute("viewListTotalElements", _list_holiday.getFirst());
