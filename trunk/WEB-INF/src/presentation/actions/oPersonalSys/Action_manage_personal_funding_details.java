...
import presentation.formbeans.oPersonalSys.Action_manage_personal_funding_details_Form;
 ...
 * 
 */
public class Action_manage_personal_funding_details extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        Action_manage_personal_funding_details_Form action_manage_personal_funding_details_Form = (Action_manage_personal_funding_details_Form) form;

        FormBeanManager fbManager = new FormBeanManager(request, action_manage_personal_funding_details_Form);

        /** 4. We obtain the initial POJO. */

        Personal personal = UseCaseFacade.ObtainPersonal(usercode, action_manage_personal_funding_details_Form
                .getPersonalcode());
        ...
        BOAdderFormBean<Personal_comment_Form> boAdderFormBean2 = action_manage_personal_funding_details_Form
                .getPersonal_comments();
        List<Personal_comment> boAdderList2 = Personal_comments_methods.process_personal_comments(boAdderFormBean2,
                fbManager, action_manage_personal_funding_details_Form.getPersonalcode(), usercode);
        ...
        fbManager = new FormBeanManager(request, Action_manage_personal_funding_details_Form.class);
        ...
        fbManager.setAttribute(personal, "personal_Form", Personal_Form.class,
                action_manage_personal_funding_details_Form.getPersonal_Form());

        /** 6.3. We add the container of the manager to the request. */

        request.setAttribute("oPersonalSys__action_manage_personal_funding_details_Form", fbManager.getContainer());
        ...
        Pair<Integer, List<Funding_detail>> _list_funding_detail = UseCaseFacade
                .ObtainAllIfunding_detail_personalFromPersonal(usercode, action_manage_personal_funding_details_Form
                        .getPersonal_Form().getPersonalcode(), filter.obtainListConfigurator(locale, false));

        /** 6.5.3. We obtain copy the list of pojos to a list of FormBeans */

        List<Funding_detail_Form> viewlistElements = ExtendedBeanUtils.copyPropertiesToFormBean(_list_funding_detail
                .getSecond(), locale, Funding_detail_Form.class);
