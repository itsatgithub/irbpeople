 ...
 *
 */
public class Action_delete_page_country extends Action {
...
Action_delete_page_country_Form action_delete_page_country_Form=(Action_delete_page_country_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_delete_page_country_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Country country = UseCaseFacade.ObtainCountry(usercode, action_delete_page_country_Form.getCountrycode());
...
fbManager=new FormBeanManager(request, Action_delete_page_country_Form.class);
	...
	fbManager.setAttribute(country, "country_Form", Country_Form.class, action_delete_page_country_Form.getCountry_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oCountrySys__action_delete_page_country_Form", fbManager.getContainer());
