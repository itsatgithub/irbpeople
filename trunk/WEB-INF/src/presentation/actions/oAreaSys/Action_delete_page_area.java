package presentation.actions.oAreaSys;
...
import presentation.formbeans.objects.*;
	...
	import presentation.formbeans.oAreaSys.*;
 ...
 *
 */
public class Action_delete_page_area extends Action {
...
Action_delete_page_area_Form action_delete_page_area_Form=(Action_delete_page_area_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_delete_page_area_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Area area = UseCaseFacade.ObtainArea(usercode, action_delete_page_area_Form.getAreacode());
...
fbManager=new FormBeanManager(request, Action_delete_page_area_Form.class);
	...
	fbManager.setAttribute(area, "area_Form", Area_Form.class, action_delete_page_area_Form.getArea_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oAreaSys__action_delete_page_area_Form", fbManager.getContainer());
