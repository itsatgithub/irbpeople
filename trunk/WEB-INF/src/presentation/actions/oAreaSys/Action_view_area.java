 ...
 *
 */
public class Action_view_area extends Action {
...
Action_view_area_Form action_view_area_Form=(Action_view_area_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_view_area_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Area area = UseCaseFacade.ObtainArea(usercode, action_view_area_Form.getAreacode());
...
fbManager=new FormBeanManager(request, Action_view_area_Form.class);
	...
	fbManager.setAttribute(area, "area_Form", Area_Form.class, action_view_area_Form.getArea_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oAreaSys__action_view_area_Form", fbManager.getContainer());
