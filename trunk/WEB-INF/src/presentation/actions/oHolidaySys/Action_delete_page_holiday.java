...
import presentation.formbeans.oHolidaySys.Action_delete_page_holiday_Form;
 ...
 *
 */
public class Action_delete_page_holiday extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Action_delete_page_holiday_Form action_delete_page_holiday_Form = (Action_delete_page_holiday_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request, action_delete_page_holiday_Form);

		/** 4.  We obtain the initial POJO. */

		Holiday holiday = UseCaseFacade.ObtainHoliday(usercode, action_delete_page_holiday_Form.getHolidaycode());
		...
		fbManager = new FormBeanManager(request, Action_delete_page_holiday_Form.class);

		/** 6.2.  We add the current POJO merged with the nested-pojo-form-bean to the manager. */

		fbManager.setAttribute(holiday, "holiday_Form", Holiday_Form.class, action_delete_page_holiday_Form.getHoliday_Form());

		/** 6.3.  We add the container of the manager to the request. */

		request.setAttribute("oHolidaySys__action_delete_page_holiday_Form", fbManager.getContainer());
