...
SELECT distinct
	...
	`dd_alumni_irb_job_positions`.`description` AS `directory_data_positions_description`,
	`dd_alumni_irb_job_position_types`.`description` AS `directory_data_position_types_description`,
				...
				AND (
					...
					OR (
						`dd_research_group_2`.`deleted` = 0
							...
							LEFT JOIN `alumni_irb_job_positions` `dd_alumni_irb_job_positions` ON (
													(
														(
															`alumni_directory_data`.`irb_job_positions` = `dd_alumni_irb_job_positions`.`alumni_irb_job_positionscode`
														)
														AND (
															(
																`dd_alumni_irb_job_positions`.`deleted` = _utf8 ''
															)
															OR (
																`dd_alumni_irb_job_positions`.`deleted` = 0
															)
														)
													)
												)
											)
											LEFT JOIN `alumni_job_position_types` `dd_alumni_irb_job_position_types` ON (
												(
													(
														`dd_alumni_irb_job_position_types`.`alumni_job_position_typescode` = `dd_alumni_irb_job_positions`.`alumni_irb_job_positionscode`
													)
													AND (
														(
															`alumni_irb_job_position_types`.`deleted` = _utf8 ''
