 ...
 *
 */
public class Action_validate_validate_personal extends Action {
...
String validationcode=((Personal) fbManager.getPOJO("personal_Form",Personal.class)).getValidationCode();

	try {
		UseCaseFacade.testValidatePersonal(usercode, personalcode, validationcode);
	...
	try {
		UseCaseFacade.validatePersonal(usercode, personalcode, action_validate_personal_Form.getObservacions());
	} catch (Exception e) {
		UserUtils.addSessionSavedAuditMessage(request, "errors.no-validation");
		return NavigationFunctions.findForward(request, mapping, "error");
	}
	
	UserUtils.addSessionSavedAuditMessage(request, "info.validationOk");
