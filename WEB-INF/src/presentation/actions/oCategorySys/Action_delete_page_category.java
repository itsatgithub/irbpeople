 ...
 *
 */
public class Action_delete_page_category extends Action {
...
Action_delete_page_category_Form action_delete_page_category_Form=(Action_delete_page_category_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_delete_page_category_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Category category = UseCaseFacade.ObtainCategory(usercode, action_delete_page_category_Form.getCategorycode());
...
fbManager=new FormBeanManager(request, Action_delete_page_category_Form.class);
	...
	fbManager.setAttribute(category, "category_Form", Category_Form.class, action_delete_page_category_Form.getCategory_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oCategorySys__action_delete_page_category_Form", fbManager.getContainer());
