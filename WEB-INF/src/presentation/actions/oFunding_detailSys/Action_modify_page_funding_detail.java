...
import presentation.formbeans.oFunding_detailSys.Action_modify_page_funding_detail_Form;
 ...
 *
 */
public class Action_modify_page_funding_detail extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Action_modify_page_funding_detail_Form action_modify_page_funding_detail_Form = (Action_modify_page_funding_detail_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request, action_modify_page_funding_detail_Form);

		/** 4.  We obtain the initial POJO. */

		Funding_detail funding_detail = UseCaseFacade.ObtainFunding_detail(usercode, action_modify_page_funding_detail_Form.getFunding_detailcode());
		...
		fbManager = new FormBeanManager(request, Action_modify_page_funding_detail_Form.class);
		...
		fbManager.setAttribute(
				...
				action_modify_page_funding_detail_Form.getFunding_detail_Form());

		/** 6.3.  We add the container of the manager to the request. */

		request.setAttribute("oFunding_detailSys__action_modify_page_funding_detail_Form", fbManager.getContainer());
		...
		Pair<Integer, List<Irb_budget>> _selec_irb_budget = UseCaseFacade.ObtainAllIrb_budget(usercode, new ListConfigurator());
		/** 6.4.6.  We copy the values to a list of formbeans. */

		List<Irb_budget_Form> _selec_irb_budget_Form = ExtendedBeanUtils.copyPropertiesToFormBean(_selec_irb_budget.getSecond(), locale, Irb_budget_Form.class);
		/** 6.4.7.  We sort the list. */
		Collections.sort(_selec_irb_budget_Form, new ToStringComparator());
		/** 6.4.8.  We put the values to the request, so the jsp can get them. */

		request.setAttribute("_selec_irb_budget", _selec_irb_budget_Form);
