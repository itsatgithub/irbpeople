 ...
 *
 */
public class Action_view_bank extends Action {
...
Action_view_bank_Form action_view_bank_Form=(Action_view_bank_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_view_bank_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Bank bank = UseCaseFacade.ObtainBank(usercode, action_view_bank_Form.getBankcode());
...
fbManager=new FormBeanManager(request, Action_view_bank_Form.class);
	...
	fbManager.setAttribute(bank, "bank_Form", Bank_Form.class, action_view_bank_Form.getBank_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oBankSys__action_view_bank_Form", fbManager.getContainer());
