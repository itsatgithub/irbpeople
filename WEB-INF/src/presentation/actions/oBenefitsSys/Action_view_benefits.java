...
import presentation.formbeans.oBenefitsSys.Action_view_benefits_Form;
 ...
 *
 */
public class Action_view_benefits extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Action_view_benefits_Form action_view_benefits_Form = (Action_view_benefits_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request, action_view_benefits_Form);

		/** 4.  We obtain the initial POJO. */

		Benefits benefits = UseCaseFacade.ObtainBenefits(usercode, action_view_benefits_Form.getBenefitscode());
		...
		fbManager = new FormBeanManager(request, Action_view_benefits_Form.class);

		/** 6.2.  We add the current POJO merged with the nested-pojo-form-bean to the manager. */

		fbManager.setAttribute(benefits, "benefits_Form", Benefits_Form.class, action_view_benefits_Form.getBenefits_Form());

		/** 6.3.  We add the container of the manager to the request. */

		request.setAttribute("oBenefitsSys__action_view_benefits_Form", fbManager.getContainer());
