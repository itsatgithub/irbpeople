...
import presentation.formbeans.oAlumni_external_jobsSys.Action_modify_page_alumni_external_jobs_Form;
 ...
 *
 */
public class Action_modify_page_alumni_external_jobs extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Action_modify_page_alumni_external_jobs_Form action_modify_page_alumni_external_jobs_Form = (Action_modify_page_alumni_external_jobs_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request, action_modify_page_alumni_external_jobs_Form);

		/** 4.  We obtain the initial POJO. */

		Alumni_external_jobs alumni_external_jobs = UseCaseFacade.ObtainAlumni_external_jobs(usercode, action_modify_page_alumni_external_jobs_Form.getAlumni_external_jobscode());
		...
		fbManager = new FormBeanManager(request, Action_modify_page_alumni_external_jobs_Form.class);

		/** 6.2.  We add the current POJO merged with the nested-pojo-form-bean to the manager. */

		fbManager.setAttribute(
				alumni_external_jobs,
				"alumni_external_jobs_Form",
				Alumni_external_jobs_Form.class,
				action_modify_page_alumni_external_jobs_Form.getAlumni_external_jobs_Form());

		/** 6.3.  We add the container of the manager to the request. */

		request.setAttribute("oAlumni_external_jobsSys__action_modify_page_alumni_external_jobs_Form", fbManager.getContainer());
