 ...
 *
 */
public class Action_view_working_hours extends Action {
...
Action_view_working_hours_Form action_view_working_hours_Form=(Action_view_working_hours_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_view_working_hours_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Working_hours working_hours = UseCaseFacade.ObtainWorking_hours(usercode, action_view_working_hours_Form.getWorking_hourscode());
...
fbManager=new FormBeanManager(request, Action_view_working_hours_Form.class);
	...
	fbManager.setAttribute(working_hours, "working_hours_Form", Working_hours_Form.class, action_view_working_hours_Form.getWorking_hours_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oWorking_hoursSys__action_view_working_hours_Form", fbManager.getContainer());
