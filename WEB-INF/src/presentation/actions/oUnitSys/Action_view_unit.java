 ...
 *
 */
public class Action_view_unit extends Action {
...
Action_view_unit_Form action_view_unit_Form=(Action_view_unit_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_view_unit_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Unit unit = UseCaseFacade.ObtainUnit(usercode, action_view_unit_Form.getUnitcode());
...
fbManager=new FormBeanManager(request, Action_view_unit_Form.class);
	...
	fbManager.setAttribute(unit, "unit_Form", Unit_Form.class, action_view_unit_Form.getUnit_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oUnitSys__action_view_unit_Form", fbManager.getContainer());
