package presentation.actions.oGrantSys;
...
import presentation.formbeans.objects.*;
	...
	import presentation.formbeans.oGrantSys.*;
 ...
 *
 */
public class Action_delete_page_grant extends Action {
...
Action_delete_page_grant_Form action_delete_page_grant_Form=(Action_delete_page_grant_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_delete_page_grant_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Grant grant = UseCaseFacade.ObtainGrant(usercode, action_delete_page_grant_Form.getGrantcode());
...
fbManager=new FormBeanManager(request, Action_delete_page_grant_Form.class);
	...
	fbManager.setAttribute(grant, "grant_Form", Grant_Form.class, action_delete_page_grant_Form.getGrant_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oGrantSys__action_delete_page_grant_Form", fbManager.getContainer());
...
Pair<Integer, List<Type_of_grant>> _selec_type_of_grant= UseCaseFacade.ObtainAllType_of_grant(usercode,  new ListConfigurator());
/** 6.4.2.  We copy the values to a list of formbeans. */

List<Type_of_grant_Form> _selec_type_of_grant_Form = ExtendedBeanUtils.copyPropertiesToFormBean(_selec_type_of_grant.getSecond(), locale, Type_of_grant_Form.class);
/** 6.4.3.  We sort the list. */
Collections.sort(_selec_type_of_grant_Form, new ToStringComparator());
/** 6.4.4.  We put the values to the request, so the jsp can get them. */

request.setAttribute("_selec_type_of_grant", _selec_type_of_grant_Form);
