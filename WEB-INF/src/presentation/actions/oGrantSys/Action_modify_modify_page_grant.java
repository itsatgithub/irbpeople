 ...
 *
 */
public class Action_modify_modify_page_grant extends Action {
...
Action_modify_modify_page_grant_Form action_modify_modify_page_grant_Form=(Action_modify_modify_page_grant_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_modify_modify_page_grant_Form);			
		
	/** 3.  We use the business logic to modify the new item */
	
	Grant grant = UseCaseFacade.UpdateGrant(usercode,(Grant) fbManager.getPOJO("grant_Form",Grant.class));	
...
return NavigationFunctions.findForward(request, mapping, "success", "grantcode="+grant.getGrantcode(), isPopUp, grant.getCode(), grant.toString());
