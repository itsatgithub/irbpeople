...
import presentation.formbeans.oAcademic_infoSys.Action_modify_page_academic_info_Form;
 ...
 *
 */
public class Action_modify_page_academic_info extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Action_modify_page_academic_info_Form action_modify_page_academic_info_Form = (Action_modify_page_academic_info_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request, action_modify_page_academic_info_Form);

		/** 4.  We obtain the initial POJO. */

		Academic_info academic_info = UseCaseFacade.ObtainAcademic_info(usercode, action_modify_page_academic_info_Form.getAcademic_infocode());
		...
		fbManager = new FormBeanManager(request, Action_modify_page_academic_info_Form.class);
		...
		fbManager.setAttribute(
				...
				action_modify_page_academic_info_Form.getAcademic_info_Form());

		/** 6.3.  We add the container of the manager to the request. */

		request.setAttribute("oAcademic_infoSys__action_modify_page_academic_info_Form", fbManager.getContainer());
		...
		Pair<Integer, List<Type_of_study>> _select_type_of_study = UseCaseFacade.ObtainAllType_of_study(usercode, new ListConfigurator());
		/** 6.4.6.  We copy the values to a list of formbeans. */

		List<Type_of_study> _selec_type_of_study_Form = ExtendedBeanUtils.copyPropertiesToFormBean(_select_type_of_study.getSecond(), locale, Type_of_study_Form.class);
