...
import presentation.formbeans.oAcademic_infoSys.Action_delete_page_academic_info_Form;
...
public class Action_mark_current_confirmation extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		Action_delete_page_academic_info_Form action_delete_page_academic_info_Form = (Action_delete_page_academic_info_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request,
				action_delete_page_academic_info_Form);

		/** 4. We obtain the initial POJO. */

		Academic_info academic_info = UseCaseFacade.ObtainAcademic_info(
				usercode,
				action_delete_page_academic_info_Form.getAcademic_infocode());
		...
		fbManager = new FormBeanManager(request,
				Action_delete_page_academic_info_Form.class);
		...
		fbManager.setAttribute(academic_info, "academic_info_Form",
				Academic_info_Form.class,
				action_delete_page_academic_info_Form.getAcademic_info_Form());

		/** 6.3. We add the container of the manager to the request. */

		request.setAttribute(
				"oAcademic_infoSys__action_delete_page_academic_info_Form",
				fbManager.getContainer());
		...
		// Pair<Integer, List<Personal>> _selec_personal=
		// UseCaseFacade.ObtainAllPersonal(usercode, new ListConfigurator());
		// /** 6.4.2. We copy the values to a list of formbeans. */
		//
		// List<Personal_Form> _selec_personal_Form =
		// ExtendedBeanUtils.copyPropertiesToFormBean(_selec_personal.getSecond(),
		...
		Pair<Integer, List<Type_of_study>> _selec_type_of_study = UseCaseFacade
				.ObtainAllType_of_study(usercode, new ListConfigurator());
		/** 6.4.6. We copy the values to a list of formbeans. */

		List<Type_of_study> _selec_type_of_study_Form = ExtendedBeanUtils
				.copyPropertiesToFormBean(_selec_type_of_study.getSecond(),
						locale, Type_of_study_Form.class);
