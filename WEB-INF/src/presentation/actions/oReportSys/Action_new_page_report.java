 ...
 *
 */
public class Action_new_page_report extends Action {
...
Action_new_page_report_Form action_new_page_report_Form=(Action_new_page_report_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_new_page_report_Form);
...
fbManager=new FormBeanManager(request, Action_new_page_report_Form.class);
	...
	fbManager.setAttribute(report, "report_Form", Report_Form.class, action_new_page_report_Form.getReport_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oReportSys__action_new_page_report_Form", fbManager.getContainer());
	
	/** 6.4.  We add the values of all (if any) of the support tables which will be used in the jsp (for selectables) to the request. */
	
	

action_new_page_report_Form.getReport_Form().getAuthor().setUsuariocode(usercode);
