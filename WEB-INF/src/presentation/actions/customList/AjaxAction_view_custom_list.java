...
public class AjaxAction_view_custom_list extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/**
		//if we are not changing the view, we take the view which we are working with:
		if(vistaName == null)
			vistaName = (String) request.getSession().getSessionValue("vistaName");
		**/
		
		/**
		//if none of the previous, we take a default view
		if (vistaName == null) {
			vistaName = "colaboradorbecaview";			
		}
		**/
		
		//finally we get the reportComObject of the choosen view and set the view in the session
		ReportComObject rco = rc.getReport(vistaName);	
		
		/**
		request.setSessionValue("vistaName", (String) vistaName);
		if (request.getSessionValue("reportCategory") == null)
