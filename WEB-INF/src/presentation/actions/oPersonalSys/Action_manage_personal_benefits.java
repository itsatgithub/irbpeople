 ...
 * 
 */
public class Action_manage_personal_benefits extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        Action_manage_personal_benefits_Form action_manage_personal_benefits_Form = (Action_manage_personal_benefits_Form) form;

        FormBeanManager fbManager = new FormBeanManager(request, action_manage_personal_benefits_Form);

        /** 4. We obtain the initial POJO. */

        Personal personal = UseCaseFacade.ObtainPersonal(usercode, action_manage_personal_benefits_Form
                .getPersonalcode());
        ...
        BOAdderFormBean<Personal_comment_Form> boAdderFormBean2 = action_manage_personal_benefits_Form
        ...
        List<Personal_comment> boAdderList2 = Personal_comments_methods.process_personal_comments(boAdderFormBean2,
                fbManager, action_manage_personal_benefits_Form.getPersonalcode(), usercode);
        ...
        fbManager = new FormBeanManager(request, Action_manage_personal_benefits_Form.class);
        ...
        fbManager.setAttribute(personal, "personal_Form", Personal_Form.class, action_manage_personal_benefits_Form
                .getPersonal_Form());

        /** 6.3. We add the container of the manager to the request. */

        request.setAttribute("oPersonalSys__action_manage_personal_benefits_Form", fbManager.getContainer());
        ...
        Pair<Integer, List<Benefits>> _list_benefits = UseCaseFacade.ObtainAllIbenefits_personalFromPersonal(usercode,
                action_manage_personal_benefits_Form.getPersonal_Form().getPersonalcode(), filter
                        .obtainListConfigurator(locale, true));

        /** 6.5.3. We obtain copy the list of pojos to a list of FormBeans */

        List<Benefits_Form> viewlistElements = ExtendedBeanUtils.copyPropertiesToFormBean(_list_benefits.getSecond(),
                locale, Benefits_Form.class);

        request.setAttribute("viewlistElements", viewlistElements);
        /** 6.5.4. We put the 'real' number of elements (without pagination) */

//        request.setAttribute("viewListTotalElements", _list_benefits.getFirst());
