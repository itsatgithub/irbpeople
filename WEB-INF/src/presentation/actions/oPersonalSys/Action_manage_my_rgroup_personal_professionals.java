 ...
 *
 */
public class Action_manage_my_rgroup_personal_professionals extends Action {
...
Action_manage_my_rgroup_personal_professionals_Form action_manage_my_rgroup_personal_professionals_Form=(Action_manage_my_rgroup_personal_professionals_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_manage_my_rgroup_personal_professionals_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Personal personal = UseCaseFacade.ObtainPersonal(usercode, action_manage_my_rgroup_personal_professionals_Form.getPersonalcode());
...
fbManager=new FormBeanManager(request, Action_manage_my_rgroup_personal_professionals_Form.class);
	...
	fbManager.setAttribute(personal, "personal_Form", Personal_Form.class, action_manage_my_rgroup_personal_professionals_Form.getPersonal_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oPersonalSys__action_manage_my_rgroup_personal_professionals_Form", fbManager.getContainer());
	...
	/** 6.5.2.  We obtain the list of objects form the Business logic */
			...
			Pair<Integer,List<Professional>> _list_professional=UseCaseFacade.ObtainAllIprofessional_personalFromPersonal(usercode,action_manage_my_rgroup_personal_professionals_Form.getPersonal_Form().getPersonalcode(),filter.obtainListConfigurator(locale, true));
