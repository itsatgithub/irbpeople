 ...
 *
 */
public class Action_manage_personal_children extends Action {
...
Action_manage_personal_children_Form action_manage_personal_children_Form=(Action_manage_personal_children_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_manage_personal_children_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Personal personal = UseCaseFacade.ObtainPersonal(usercode, action_manage_personal_children_Form.getPersonalcode());
...
fbManager=new FormBeanManager(request, Action_manage_personal_children_Form.class);
	...
	fbManager.setAttribute(personal, "personal_Form", Personal_Form.class, action_manage_personal_children_Form.getPersonal_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oPersonalSys__action_manage_personal_children_Form", fbManager.getContainer());
	...
	/** 6.5.2.  We obtain the list of objects form the Business logic */
			...
			Pair<Integer,List<Child>> _list_child=UseCaseFacade.ObtainAllIchild_personalFromPersonal(usercode,action_manage_personal_children_Form.getPersonal_Form().getPersonalcode(),filter.obtainListConfigurator(locale, false));
	
	/** 6.5.3.  We obtain copy the list of pojos to a list of FormBeans */
	
	List<Child_Form> viewlistElements =ExtendedBeanUtils.copyPropertiesToFormBean(_list_child.getSecond(), locale, Child_Form.class);
