 ...
 *
 */
public class Action_manage_my_rgroup_personal_funding_details extends Action {
...
Action_manage_my_rgroup_personal_funding_details_Form action_manage_my_rgroup_personal_funding_details_Form=(Action_manage_my_rgroup_personal_funding_details_Form) form;

FormBeanManager fbManager=new FormBeanManager(request, action_manage_my_rgroup_personal_funding_details_Form);
	
	/** 4.  We obtain the initial POJO. */
	
	Personal personal = UseCaseFacade.ObtainPersonal(usercode, action_manage_my_rgroup_personal_funding_details_Form.getPersonalcode());
...
fbManager=new FormBeanManager(request, Action_manage_my_rgroup_personal_funding_details_Form.class);
	...
	fbManager.setAttribute(personal, "personal_Form", Personal_Form.class, action_manage_my_rgroup_personal_funding_details_Form.getPersonal_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oPersonalSys__action_manage_my_rgroup_personal_funding_details_Form", fbManager.getContainer());
	...
	/** 6.5.2.  We obtain the list of objects form the Business logic */
			...
			Pair<Integer,List<Funding_detail>> _list_funding_detail=UseCaseFacade.ObtainAllIfunding_detail_personalFromPersonal(usercode,action_manage_my_rgroup_personal_funding_details_Form.getPersonal_Form().getPersonalcode(),filter.obtainListConfigurator(locale, false));
