 ...
 * 
 */
public class Action_manage_personal_grant_concessions extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        Action_manage_personal_grant_concessions_Form action_manage_personal_grant_concessions_Form = (Action_manage_personal_grant_concessions_Form) form;

        FormBeanManager fbManager = new FormBeanManager(request, action_manage_personal_grant_concessions_Form);

        /** 4. We obtain the initial POJO. */

        Personal personal = UseCaseFacade.ObtainPersonal(usercode, action_manage_personal_grant_concessions_Form
                .getPersonalcode());
        ...
        BOAdderFormBean<Personal_comment_Form> boAdderFormBean2 = action_manage_personal_grant_concessions_Form
        ...
        List<Personal_comment> boAdderList2 = Personal_comments_methods.process_personal_comments(boAdderFormBean2,
                fbManager, action_manage_personal_grant_concessions_Form.getPersonalcode(), usercode);
        ...
        fbManager = new FormBeanManager(request, Action_manage_personal_grant_concessions_Form.class);
        ...
        fbManager.setAttribute(personal, "personal_Form", Personal_Form.class,
                action_manage_personal_grant_concessions_Form.getPersonal_Form());

        /** 6.3. We add the container of the manager to the request. */

        request.setAttribute("oPersonalSys__action_manage_personal_grant_concessions_Form", fbManager.getContainer());
        ...
        Pair<Integer, List<Grant_concession>> _list_grant_concession = UseCaseFacade
                .ObtainAllIgrant_concession_personalFromPersonal(usercode,
                        action_manage_personal_grant_concessions_Form.getPersonal_Form().getPersonalcode(), filter
                                .obtainListConfigurator(locale, false));

        /** 6.5.3. We obtain copy the list of pojos to a list of FormBeans */

        List<Grant_concession_Form> viewlistElements = ExtendedBeanUtils.copyPropertiesToFormBean(
                _list_grant_concession.getSecond(), locale, Grant_concession_Form.class);
