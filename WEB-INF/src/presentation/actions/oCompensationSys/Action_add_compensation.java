 ...
 *
 */
public class Action_add_compensation extends Action {
...
Action_add_compensation_Form action_add_compensation_Form=(Action_add_compensation_Form) form;


FormBeanManager fbManager=new FormBeanManager(request, action_add_compensation_Form);
...
fbManager=new FormBeanManager(request, Action_add_compensation_Form.class);
	...
	fbManager.setAttribute(compensation, "compensation_Form", Compensation_Form.class, action_add_compensation_Form.getCompensation_Form());
	
	/** 6.3.  We add the container of the manager to the request. */
	
	request.setAttribute("oCompensationSys__action_add_compensation_Form", fbManager.getContainer());
...
List<Type_of_compensation_Form> _selec_type_of_compensation_Form = ExtendedBeanUtils.copyPropertiesToFormBean(_selec_type_of_compensation.getSecond(), locale, Type_of_compensation_Form.class);
...
request.setAttribute("_selec_type_of_compensation", _selec_type_of_compensation_Form);
	...
	Compensation lastCompensation=UseCaseFacade.ObtainCurrentCompensationFromPersonal(usercode, action_add_compensation_Form.getCompensation_Form().getCompensation_personal().getPersonalcode());
	if(lastCompensation!=null){
		request.setAttribute("showAmountPossibilities", "true");
	}
