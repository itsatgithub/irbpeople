...
import presentation.formbeans.oCompensationSys.Action_modify_page_compensation_Form;
 ...
 *
 */
public class Action_modify_page_compensation extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Action_modify_page_compensation_Form action_modify_page_compensation_Form = (Action_modify_page_compensation_Form) form;

		FormBeanManager fbManager = new FormBeanManager(request, action_modify_page_compensation_Form);

		/** 4.  We obtain the initial POJO. */

		Compensation compensation = UseCaseFacade.ObtainCompensation(usercode, action_modify_page_compensation_Form.getCompensationcode());
		...
		fbManager = new FormBeanManager(request, Action_modify_page_compensation_Form.class);

		/** 6.2.  We add the current POJO merged with the nested-pojo-form-bean to the manager. */

		fbManager.setAttribute(compensation, "compensation_Form", Compensation_Form.class, action_modify_page_compensation_Form.getCompensation_Form());

		/** 6.3.  We add the container of the manager to the request. */

		request.setAttribute("oCompensationSys__action_modify_page_compensation_Form", fbManager.getContainer());
